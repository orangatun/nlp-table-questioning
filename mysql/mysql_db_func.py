...
def add_file(user_id, file_name, file_path):
    ...
    try:
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO `csv_file` (`user_id`, `file_name`, `file_path`) VALUES (%d, %s, %s) RETURNING id",
...
def get_file_id_by_name(user_id, file_name):
    ...
    cursor = db.cursor()
    cursor.execute(
        'SELECT `id` FROM `csv_file` WHERE `user_id` = %d AND `file_name` = %s ORDERBY `uploaded` DESC', (user_id, file_name)
    )
...
def add_question(file_id, question):
    ...
    try:
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO `question` (`file_id`, `question`) VALUES (%d, %s) RETURNING id",
            (file_id, question),
        )
        inserted_id = cursor.fetchone()['id']
        db.commit()
...
def add_response_to_question(answer, question_id):
    ...
    try:
        db.cursor().execute(
            "UPDATE `question` SET `response` = %s WHERE `id` = %d", 
            (answer, question_id),
        )
        db.commit()
    except db.Error:
...
def get_questions_by_user_id(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        'SELECT f.id as file_id, f.file_name as file_name, f.uploaded as uploaded, q.id as question_id, q.req_time as question_time, q.question as question, q.response as response FROM user u JOIN csv_file f ON u.id = f.user_id LEFT JOIN question q ON f.id = q.file_id WHERE u.id = %d ORDER BY u.id, f.id, q.id', (user_id,)
    )
